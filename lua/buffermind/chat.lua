-- ~/.config/nvim/lua/llm_chat/chat.lua
local M = {}
local config = require("llm_chat.config").get()
...
function M.open_chat_buffer()
  ...
  vim.api.nvim_buf_set_name(0, "Chat with " .. config.platform)
  ...
  vim.api.nvim_buf_set_keymap(
    ...
    "n",
    "<C-]>",
    '<cmd>lua require("llm_chat.chat").send_message()<CR>',
              ...
              if success and decoded_chunk then
                ...
                if content then
                  if vim.api.nvim_buf_is_valid(buffer) then
                    local current_lines = vim.api.nvim_buf_get_lines(buffer, response_start_line - 1, -1, false)
                    ...
                    vim.api.nvim_buf_set_lines(buffer, response_start_line - 1, -1, false, current_lines)

                    local last_line_num = vim.api.nvim_buf_line_count(buffer)
                    local last_col = #vim.api.nvim_buf_get_lines(buffer, last_line_num - 1, last_line_num, false)[1]
